...
function renderConnectionPage() {
  ...
  const emailInviteBlock = child
    ? `<div class="email-invite-status">${icon('mail')}<div><strong>Convites para este e-mail</strong><span>${state.emailInvites.length ? `${state.emailInvites.length} convite(s) aguardando aceite.` : 'Nenhum convite recebido ainda.'}</span></div><button class="ghost-button" data-action="refresh-connections" type="button">Atualizar</button></div>${state.emailInvites.map((invite) => `<div class="email-invite-pending"><div><strong>Convite de família</strong><span>Este convite expira em ${new Date(invite.expires_at).toLocaleString('pt-BR')}.</span></div><button class="primary-button" data-action="accept-email-invite" data-invite-id="${invite.id}" type="button">${icon('check')} Aceitar</button></div>`).join('')}`
